...
async function cleanupResolveInterface() {
    ...
    if (!isSuccess) {
        debugLog('Error: Failed to cleanup Resolve interface!');
    }
...
async function renderTimeline(event, renderPresetName, targetDirPath, targetClipName) {
    project = await getCurrentProject();
    ...
    let isSuccess = await project.LoadRenderPreset(renderPresetName);
    if (!isSuccess) return false;

    // Set render settings
    isSuccess = await project.SetRenderSettings({ "TargetDir": targetDirPath, "CustomName": targetClipName });
    if (!isSuccess) return false;
