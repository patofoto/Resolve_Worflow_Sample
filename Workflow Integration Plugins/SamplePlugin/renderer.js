...
async function addClips() {
    ...
    const clips = await window.resolveAPI.addClips(selectedClipsToAdd)
    if (!clips) {
        alert(`Error: Failed to add clips: ${selectedClipsToAdd}`);
        return;
    }

    console.log(`Added clips: ${selectedClipsToAdd}`);
}
...
async function renderTimeline() {
    ...
    const isSuccess = await window.resolveAPI.renderTimeline(renderPresetName, targetDirPath, targetClipName);
    if (!isSuccess) {
        alert(`Error: Failed to render current timeline`);
        return;
    }

    console.log(`Render timeline started`);
}
